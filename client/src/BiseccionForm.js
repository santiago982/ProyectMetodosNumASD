...
function BiseccionForm() {
  ...
  const renderChart = () => {
    if (root !== null) {
      ...
      const xValues = [...Array(100).keys()].map(x => parseFloat(a) + (parseFloat(b) - parseFloat(a)) * x / 100);
  ...
  return (
    <div className="container mt-5">
      ...
      <div className="position-relative border border-2 border-primary rounded-3" style={{ width: '1000px', height: '500px' }}>
        <canvas id="myChart" className="position-absolute" style={{ zIndex: '1' }}></canvas>
        <div ref={xAxisLineRef} className="position-absolute border-bottom border-danger" style={{ width: '80%', left: '10%' }}></div>
        <div ref={yAxisLineRef} className="position-absolute border-end border-danger" style={{ height: '80%', top: '10%' }}></div>
      </div>
      ...
      <div className="mt-5">
        ...
        <div className="row">
          ...
          <div className="col d-flex align-items-end">
            <button onClick={renderPolynomialChart} className="btn btn-primary">Calcular</button>
          </div>
        </div>
        <canvas id="polynomialChart"></canvas>        
      </div>
      <div className="container mt-5">
        {/* Formulario para el método de Simpson */}
        <Form handleChartData={handleChartData} />

        {/* Renderizar el gráfico aquí */}
        {chartData && (
          <div className="mt-5">
            <h2>Gráfico</h2>
            {/* Aquí renderiza el gráfico utilizando los datos recibidos */}
          </div>
        )}
      </div>
    </div>
