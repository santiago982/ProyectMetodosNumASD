...
def simpson(funcion, a, b, n):
    if n == 2:
        ...
        resultado = h * (funcion(valores[0]) + 4*funcion(valores[1]) + funcion(valores[-1]))# Calcula la aproximación de la integral utilizando la fórmula de Simpson simple. Multiplica el ancho del subintervalo por la suma ponderada de los  valores de la función en los extremos y el punto medio del intervalo.
        print(f"El resultado del método es = {round(resultado, 5)}")
        return resultado
    
     # si es mas de 2 usa la formula 2 o caso2
    elif n > 2 and n % 2 == 0:
        h = (b - a) / n  # Calcula el ancho de cada subintervalo para el método compuesto.
        ...
        resultado = h / 3 * (sumatoria_xi + sumatoria_mi) #  Calcula la aproximación de la integral utilizando la fórmula compuesta del método de Simpson.
        print(f"El resultado del método es = {round(resultado, 5)}")
...
if resultado is not None:
    ...
    ax.fill_between(x_area, y_area, alpha=0.2, color='green', label='Área bajo la curva')
    ...
    ax.set_xlabel('x')
    ax.set_ylabel('f(x)')
    ax.set_title(f'Gráfica de la función y Área bajo la curva con {n} divisiones y líneas')
